...
const AllGroups = document.getElementById('AllGroups');
    ...
    try {
      const token = localStorage.getItem('token')
        const groups = await axios.get('http://51.21.12.32:3000/group/AllMembers',{headers: { "Authorization": token}});
...
async function IsMember(groupId, Name) {
    try{
    const token = localStorage.getItem('token')
   const result = await axios.get(`http://51.21.12.32:3000/Group/IsMember/${groupId}`,{ headers: { "Authorization": token } } )
...
async function sendmessage(event){
  ...
  const response = await axios.post('http://51.21.12.32:3000/group/chats', content, {headers:{'Authorization':token}})
...
async function ShowUserChatsOnScreen(id){
  ...
  const groupdata = await axios.get(`http://51.21.12.32:3000/particulargroup/chats/${id}`,{headers:{'Authorization':token}})
...
async function ShowMessagesOnScreen(groupId){
  ...
  const databasedata = await axios.get(`http://51.21.12.32:3000/server/Groupmessages/${groupId}?lastMessageId=${lastMessageId}`, { headers: { "Authorization": token } })
...
async function userIsAdmin(groupId){
  const token = localStorage.getItem('token')
   const isAdmin = await axios.get(`http://51.21.12.32:3000/check/isAdmin/${groupId}`, { headers: { "Authorization": token } })
...
async function uploadFile(event){
  ...
  formData.append('file', uploadedFile)
   ...
   const data =  await axios.post('http://51.21.12.32:3000/groupchat/add-file', formData, {headers: {
      'Authorization': token,
