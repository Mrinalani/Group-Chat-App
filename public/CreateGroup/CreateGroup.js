...
function createGroup(groupName){
    ...
    axios.post('http://13.49.249.217:3000/group/createGroup',data,{headers:{'Authorization':token}}).then((res)=>{
...
addParticipantBtn.addEventListener('click', async (e) => {
   ...
   const isAdmin = await axios.get(`http://13.49.249.217:3000/user/isAdmin/${groupId}`,{headers:{'Authorization':token}})
      if(isAdmin.data.isAdmin === false){
         alert('Only Admin Can Add Participants')
      }
      else{
        console.log(isAdmin)
    const allusers = await axios.get('http://13.49.249.217:3000/users/getAll');
    const usersData = allusers.data.participants;
    console.log("getting all users =",usersData)

    const groupMembersResponse = await axios.get(`http://13.49.249.217:3000/groups/${groupId}/members`,);
    ...
    submitBtn.addEventListener('click', () => {
        ...
        axios.post('http://13.49.249.217:3000/add/participants', {
       ...
       const removeParticipants = axios.post('http://13.49.249.217:3000/remove/participants',{
...
async function makeAdmin(userId,groupId){
    ...
    const adminMember = await axios.post('http://13.49.249.217:3000/user/makeAdmin',Ids)
