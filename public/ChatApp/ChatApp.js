...
const activeusers = await axios.get('http://51.21.12.32:3000/get/activeusers')
...
const updateactive = await axios.put(`http://51.21.12.32:3000/user/active/${decodedtoken.id}`)
...
async function sendmessage(event){
    ...
    const response = await axios.post('http://51.21.12.32:3000/user/message', content,  { headers: { "Authorization": token } })
...
async function ShowMessagesOnScreen(){
    ...
    const databasedata = await axios.get(`http://51.21.12.32:3000/server/messages?lastMessageId=${lastMessageId}`, { headers: { "Authorization": token } })
...
async function logoutUserFromDB(obj){
    ...
    const token = localStorage.getItem('token');
    const update = await axios.put('http://51.21.12.32:3000/update',obj, { headers: { "Authorization": token } })
    console.log('>>>>>>>>>>>>>>>',update)
...
async function uploadFile(event){
   ...
   // formData.append('groupId', groupId)
    formData.append('file', uploadedFile)
     ...
     const data =  await axios.post('http://51.21.12.32:3000/chat/add-file', formData, {headers: {
        'Authorization': token,
